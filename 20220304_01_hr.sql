...
HR	TEST6_COL2_PK	TBL_TEST6	U	COL2		
*/
...
--   +
ALTER TABLE TBL_TEST7
ADD CONSTRAINT TEST7_COL1_UK UNIQUE(COL2);
--  ↓
ALTER TABLE TBL_TEST7
ADD (
      CONSTRAINT TEST7_COL1_PK PRIMARY KEY(COL1)
    , CONSTRAINT TEST7_COL1_UK UNIQUE(COL2)
    );
...
HR	TEST7_COL1_UK	TBL_TEST7	U	COL2		
*/
...
-- 또한, 컬럼에서 수정되는 데이터를 검사하여 조건에 맞는 데이터로 수정되는것만
-- 허용하는 기능을 수행하게 된다.
...
--==>>
/*
HR	SYS_C007093	TBL_TEST8	C	COL3	COL3 BETWEEN 0 AND 100	
HR	SYS_C007094	TBL_TEST8	P	COL1		
*/
...
-- SSN 컬럼(주민번호 컬럼)에서
-- 데이터 임력 시 성별이 유효한 데이터만 입력될 수 있도록
-- 체크 제약조건을 추가할 수 있도록 한다.
-- (→ 주민번호 특정 자리에 입력 가능한 데이터를 1,2,3,4만 가능하도록 처리한다.)
...
ADD (
 CONSTRAINT TBL_TESTMEMBER_SID_PK PRIMARY KEY(SID)
,CONSTRAINT TBL_TESTMEMBER_SSN_CK CHECK((SUBSTR(SSN,8,1) IN ('1','2','3','4')) AND (SSN LIKE '______-_______'))
);
...
HR	TBL_TESTMEMBER_SSN_CK	TBL_TESTMEMBER	C	SSN	(SUBSTR(SSN,8,1) IN ('1','2','3','4')) AND (SSN LIKE '______-_______')	
*/
...
INSERT INTO TBL_TESTMEMBER(SID, NAME, SSN, TEL) 
VALUES(6, '양윤정', '961112-6234567','010-1111-1111'); --> 에러 발생
...
-- 이 떄, 자식 테이블에 FOREIGN KEY 제약조건이 설정된다.
...
--==>>
/*
HR	SYS_C007103	TBL_EMP1	P	SID		
HR	SYS_C007104	TBL_EMP1	R	JIKWI_ID		NO ACTION
*/
...
INSERT INTO TBL_EMP1(SID,NAME, JIKWI_ID) VALUES(6,'오이삭',NULL); -- 7과 같은 구문
INSERT INTO TBL_EMP1(SID,NAME) VALUES(7,'박현지'); -- 6과 같은 구문
...
-- 부모 테이블의 부장 데이터를 참조하고 있는
-- 자식 테이블의 데이터가 존재하지 않는 상황.
...
WHERE JIKWI_ID = 1;
--==>> 에러 발생
-- (ORA-02292: integrity constraint (HR.SYS_C007104) violated - child record found)
...
INSERT INTO TBL_TEST11(COL1,COL2) VALUES(3,NULL);   --> 에러발생
INSERT INTO TBL_TEST11(COL1) VALUES(3);             --> 에러발생
...
--==>>
/*
HR	SYS_C007112	TBL_TEST11	C	COL2	"COL2" IS NOT NULL	
HR	SYS_C007113	TBL_TEST11	P	COL1		
*/
...
--※ NOT NULL 제약조건만 TBL_TEST13 테이블의 COL2 애 추가하는 경우
-- 다음과 같은 방법을 사용하는 것도 가능하다.
ALTER TABLE TBL_TEST13
MODIFY COL2 NOT NULL;
--==>>Table TBL_TEST13이(가) 변경되었습니다.

-- 컬럼 레벨에서 NOT NULL 제약조건을 지정한 테이블(TBL_TEST12)
DESC TBL_TEST12;
--==>>
/*
이름   널?       유형           
